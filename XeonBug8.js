...
const xtime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
        ...
        const time2 = moment().tz('Asia/Kolkata').format('HH:mm:ss')  
         if(time2 < "23:59:00"){
var xeonytimewisher = `This is ERROR-X☠💀☠ created by EMMY HENZ. It's late master get some rest and you account is secured by ERROR-X master`
 }
 if(time2 < "19:00:00"){
var xeonytimewisher = `This is ERROR-X☠💀☠ created by EMMY HENZ👑. The day is almost drawing near. ERROR-X would keep you updated master💯`
 }
 if(time2 < "18:00:00"){
var xeonytimewisher = `This is ERROR-X created by EMMY HENZ👑. ERROR-X WONT FAIL TO KEEP YOUR ACCOUNT SAFE🤭`
 }
 if(time2 < "15:00:00"){
var xeonytimewisher = `This is ERROR-X☠💀☠ created by EMMY HENZ👑. GOOD AFTERNOON. The sun is hot, you should take a nap and dont forget your account is highly protected by ERROR-X`
 }
 if(time2 < "11:00:00"){
var xeonytimewisher = `Good Morning 🌄 MASTER. Your account was protected. While you were asleep 25587xxxxxx tried to send you a bug message but ERROR-X protected your account💯`
 }
 if(time2 < "05:00:00"){
var xeonytimewisher = `This is ERROR-X☠💀☠ created by EMMY HENZ👑. Hope you slept well, ERROR-X IS STILL ACTIVE. Good morning 🤭`
 } 
...
senddocu(buffer)
    ...
    case 'autostatusview':{
            ...
            case 'autobio':
                ...
                if (q == 'on') {
                    autobio = true
                    replygcxeon(`EMMY HENZ has Successfully Changed AutoBio To ${q}`)
                } else if (q == 'off') {
                    autobio = false
                    replygcxeon(`EMMY HENZ has Changed AutoBio To ${q}`)
                }
                break
            case 'mode':
                if (!isCreator) return replygcxeon(mess.owner)
                    ...
                    XeonBotInc.sendMessage(i, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                ...
                                thumbnailUrl: 'https://i.ibb.co/MRLdRxw/IMG-20240913-WA0130-1.jpg',
            ...
            case 'resetlink':
                ...
                await XeonBotInc.groupRevokeInvite(m.chat)
                    .then(res => {
                        replygcxeon(`EMMY HENZ has Successful Reset, Group Invite Link ${groupMetadata.subject}`)
...
_ERROR-X NodeJS Memory Usage🚧_
${Object.keys(used).map((key, _, arr) => `${key.padEnd(Math.max(...arr.map(v=>v.length)),' ')}: ${formatp(used[key])}`).join('\n')}

${cpus[0] ? `_ERROR-X Total CPU Usage🧑‍🔧_
...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with EMMY HENZ TECH👑😉👉09121785627`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
            ...
            break
            case 'runtime':
                let runtimetext = `ERROR-X 𝐇𝐚𝐯𝐞 𝐛𝐞𝐞𝐧 𝐫𝐮𝐧𝐧𝐢𝐧𝐠 𝐟𝐨𝐫 ${runtime(process.uptime())}`
                XeonBotInc.sendMessage(m.chat, {
                    text: runtimetext,
                    contextInfo: {
                        externalAdReply: {
                ...
                XeonBotInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on YouTube @EMMANUEL HENSHAW`,
                            thumbnailUrl: 'https://i.ibb.co/MRLdRxw/IMG-20240913-WA0130-1.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
            ...
            case 'donasi':
                ...
                XeonBotInc.sendMessage(m.chat, {
                    text: 'PALMPAY🏦 Acct No: 9125042727😊 Acct Name: OFFIONG HENSHAW\n\n' + textnate
                }, {
                    quoted: m
                })
                break
...
XeonBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, Hi ${pushname} . My great owner is EMMY HENZ👑, HE CODED ME`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
...
case "xandroid": {
  ...
  if (cleanedNumber == "2349125042727") {
    return;
  }
  ...
  sendMessageWithMentions(
    ...
    " Using *" + command + "* ✅\n\nPause for 2mins so that  ERROR-X☠💀☠҉ doesn't get your account banned✅. Thank you for choosing ERROR-X", 
    [whatsappNumber]
  );
}
break;
...
case "xios2":
  ...
  case "xgc":
  {
    ...
    try {
      ...
      await sleep(2500); // Adjusted sleep time for clarity
      replygcxeon("*DONE✅ EMMY HENZ HAS BEEN SENT TO THE GROUP!.*");
...
await replygcxeon(`In process....`)
            ...
            sendMessageWithMentions(
    "Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* ✅\n\nPause for 2mins so that ERROR-X doesn't get your account banned✅. Thank you for choosing ERROR-X☠💀☠", 
    [whatsappNumber]
  );
            }
				...
				case 'xpayment': {
					if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Usage .${command} 2349125042727`)
let cleanedNumber = text.replace(/[^0-9]/g, '');
if (cleanedNumber.startsWith('0')) return replygcxeon(`Example : ${prefix+command} 916909137213`)
var contactInfo = await XeonBotInc.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
  let whatsappNumber = cleanedNumber + '@s.whatsapp.net';
  ...
  async function xeonBugPay(jid){
            ...
            sendMessageWithMentions(
    "Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* ✅\n\nPause for 2mins so that ERROR-X☠💀☠ doesn't get your account banned✅. Thank you for choosing ERROR-X☠💀☠҉", 
    [whatsappNumber]
  );
				}
				break;
...
if (!isPremium) return replygcxeon(mess.prem)
if (!q) return replygcxeon(`Usage .${command} 2349125042727`)
...
await replygcxeon(`EMMY HENZ has Successfully Send Bug to ${xeonyvictim} Using ${command}. ✅`)
}
break
...
case 'lockotp': case 'tempban': {
  ...
  sendMessageWithMentions(
    "EMMY HENZ HAS SUCCESSFULLY ACTIVATED OTP LOCK😅 To @" + xeonMention.split('@')[0] + 
    " Using *" + command + "* ✅\n\nPause for 2mins so that  ERROR-X doesn't get your account banned✅. Thank you for choosing ERROR-X☠💀☠", 
    [xeonMention]
  );
  try {
...
_*RATED 18+🚫.....USE WISELY*_

©POWERED BY ERROR-X || EMMYHENZ`
if (typemenu === 'v1') {
                    XeonBotInc.sendMessage(m.chat, {
                        text: xeonmenuoh,
                        contextInfo: {
